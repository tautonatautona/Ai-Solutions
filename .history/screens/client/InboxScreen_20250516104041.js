...
const InboxScreen = ({ navigation }) => {
  ...
  const deleteConversation = async (conversationId) => {
    try {
      ...
      // Filter out messages belonging to the conversation to delete
      messages = messages.filter(msg => msg.conversationId !== conversationId);

      // Update the document with filtered messages
      await db.collection('chatMessages').doc(userId).set({ messages });
