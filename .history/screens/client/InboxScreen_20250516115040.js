    ...
    try {
      ...
      for (const [convId, messages] of convMessagesMap.entries()) {
        ...
        if (latestMessage.sender === 'bot') {
          type = 'AI';
          // For bot sender, display the last message sent by the bot in this conversation
          // Find the last message sent by bot in messages array
          const lastBotMessage = messages.slice().reverse().find(msg => msg.sender === 'bot');
          if (lastBotMessage) {
            convMap.set(convId, {
              id: convId,
              preview: lastBotMessage.text || '',
              type,
              lastSender: lastBotMessage.sender || null,
              date: lastBotMessage.timestamp ? (lastBotMessage.timestamp.toDate ? lastBotMessage.timestamp.toDate() : new Date(lastBotMessage.timestamp)) : new Date(),
              recipientId: lastBotMessage.recipientId || lastBotMessage.recipient || null,
            });
            continue;
          }
