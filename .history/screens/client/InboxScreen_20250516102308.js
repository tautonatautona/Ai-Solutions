...
const InboxScreen = ({ navigation }) => {
  ...
  const fetchConversations = async () => {
    ...
    try {
      ...
      const escalationsDocRef = doc(db, 'escalations', userId);
      const escalationsDocSnap = await getDoc(escalationsDocRef);
      ...
      if (escalationsDocSnap.exists()) {
        const escalationsData = escalationsDocSnap.data();
        escalationMessages = escalationsData.messages || [];
      }

      console.log('Escalation messages:', escalationMessages);
      console.log('Escalation messages senders:', escalationMessages.map(m => m.sender));
      console.log('Combined messages count:', chatMessages.length + escalationMessages.length);
      console.log('Sample combined messages:', [...chatMessages, ...escalationMessages].slice(0, 5));

      // Combine messages from both collections
      const messages = [...chatMessages, ...escalationMessages];
      ...
      for (const message of messages) {
        ...
        // Fix: Ensure messages from staff are included by checking sender and recipient roles
        // Also, if sender is staff, set recipientId to userId to ensure proper display
        if (message.sender === 'staff' && !message.recipientId) {
