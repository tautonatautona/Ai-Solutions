...
const InboxScreen = ({ navigation }) => {
  ...
  const fetchConversations = async () => {
    ...
    try {
      ...
      for (const message of combinedMessages) {
        const convId = message.conversationId || message.chatId || 'unknown';
        ...
        if (!convMap.has(convId)) {
          convMap.set(convId, {
            ...
            date: message.timestamp ? (message.timestamp.toDate ? message.timestamp.toDate() : new Date(message.timestamp)) : new Date(),
            recipientId: message.recipientId || message.recipient || null,
          });
        } else {
          const existing = convMap.get(convId);
          const currentDate = message.timestamp ? (message.timestamp.toDate ? message.timestamp.toDate() : new Date(message.timestamp)) : new Date();
  ...
  const handleConversationPress = (conversationId, recipientId) => {
    ...
    if (conversation && conversation.lastTwoSenders) {
      ...
      console.log('Last two senders:', lastTwo);
      if (lastTwo.length === 2) {
        // Adjusted condition to check if last two messages are from bot and user (in any order)
        if (
          (lastTwo.includes('bot') && lastTwo.includes('user')) ||
          (lastTwo.includes('bot') && lastTwo.includes('staff'))
        ) {
          if (conversationId) {
