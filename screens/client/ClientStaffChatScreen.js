...
import { collection, query, where, onSnapshot, addDoc, orderBy, serverTimestamp, doc, getDoc, setDoc } from 'firebase/firestore';
...
const ClientStaffChatScreen = ({ route, navigation }) => {
  ...
  const [recipientBotName, setRecipientBotName] = useState(null);
  ...
  const renderMessageItem = ({ item }) => {
    ...
    if (item.createdAt) {
      // Try parsing timestamp string like "16 May 2025 at 10:05:13 UTC+2"
      if (typeof item.createdAt === 'string') {
        // Remove "at" and "UTC" parts to parse easily
        const cleanedTimestamp = item.createdAt.replace(' at ', ' ').replace(/UTC.*/, '');
        messageDate = new Date(cleanedTimestamp);
      } else if (item.createdAt.toDate && typeof item.createdAt.toDate === 'function') {
        messageDate = item.createdAt.toDate();
      } else if (typeof item.createdAt === 'object' && item.createdAt.seconds) {
        messageDate = new Date(item.createdAt.seconds * 1000);
    ...
    const timeString = messageDate && !isNaN(messageDate.getTime()) ? messageDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) : '';
    return (
      <View
        style={[
          styles.message,
